...
import { updateBtp, updateFdp, updateMtp, updatePatents, updatePhd, updateProjectgrands, updateStc } from '../../redux/recordsRedux';
...
const Project = () => {
  ...
  return (
    <Container>
      <Top>
        <span>Projects</span>
        <Link to='/projectgrands/edit' >
          <button><AddIcon/> Add New</button>
        </Link>
      </Top>
      <Bottom>
      {fetching===false&&projectList.length===0?
        <EmptyList qoute={'Nothing to show here. Please add your Granted Projects'}/>
        :''}
        {fetching===false?
        projectList.map((scholars)=>
          <Entry>
            <section>
            <Link to="/projectgrands/edit" state={scholars}>
              <EditIcon/>
            </Link>
            </section>
